...
//}
//
//Server * Server::CreateTCP(u_short port)
//{
...
//}
//
//Server * Server::CreateUDP(u_short port)
//{
//	WORD sockVer = MAKEWORD(2, 2);
//	WSADATA wsaData;
//	if (WSAStartup(sockVer, &wsaData))
...
//	Server *server = new Server();
//
//	server->m_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
//	if (server->m_socket == INVALID_SOCKET)
//	{
//		cout << "套接字初始化失败" << endl;
//		delete server;
...
//		m_listMt.unlock();
//
//		cout << "接收到来自" << inet_ntoa(clientAddr.sin_addr) << "的连接" << endl;
//
//		//ServerLog serverLog("接受到来自" + string(inet_ntoa(clientAddr.sin_addr)) + "的连接", "test_log.log");
...
//				string password = data.data.registered.password;
//
//				if (FindClientByName(userName) != nullptr)	//如果重名了
//				{
//					RESPONSE response;
//					response.data.error.flag = RE_ERROR;
//					strcpy(response.data.error.errorMsg, "用户名已存在。");
//					send(pClient->GetSocket(), response.ruler, sizeof(response.data.error), 0);
//				}
//				else
//				{
//					if (pClient->SetName(userName, password))
//					{
//						RESPONSE response;
//						response.data.error.flag = RE_ERROR;
//						strcpy(response.data.error.errorMsg, "注册失败，请检查输入格式！");
//						send(pClient->GetSocket(), response.ruler, sizeof(response.data.error), 0);
